...
#include <cassert>
#include "ActMap.h"
#include "../../Injection/D2Specifics/D2Common.h"
...
bool ActMap::SpaceIsWalkable(const Point& point, bool abs) const {
    ...
    return !(SpaceHasFlag(ActMap::Avoid, point, abs) || SpaceHasFlag(ActMap::BlockWalk, point, abs) || SpaceHasFlag(ActMap::BlockPlayer, point, abs) ||
             SpaceHasFlag(ActMap::NPCCollision, point, abs) || SpaceHasFlag(ActMap::Object, point, abs) //    ||
...
bool ActMap::SpaceIsInvalid(const Point& point, bool abs) const {
...
void ActMap::DumpLevel(const char* file) const {
    ...
    for (Room2* room = level->pRoom2First; room; room = room->pRoom2Next) {
        for (PresetUnit* preset = room->pPreset; preset; preset = preset->pPresetNext) {
            ...
            fprintf(f, "Preset %d: at (%d, %d, relative: %d, %d) of type %d, txtFile %d, _1 %d, _3 %d, room (%d, %d) %s\n", i++, loc.first, loc.second, rel.first,
                    rel.second, preset->dwType, preset->dwTxtFileNo, preset->_1, preset->_3, room->dwPosX, room->dwPosY, wName);
        }
    }
