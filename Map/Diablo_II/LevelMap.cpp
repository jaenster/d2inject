        ...
        for (int j = x; j < height && p != map->pMapEnd; j++, p++) {
            if ((*p & LevelMap::ClosedDoor) == LevelMap::ClosedDoor || (*p & LevelMap::Player) == LevelMap::Player) {
                SetCollisionData(j, i, 0);
                        ...
                        (*p & LevelMap::NPCLocation) == LevelMap::NPCLocation || (*p & LevelMap::DeadBody) == LevelMap::DeadBody ||
                        (*p & LevelMap::Item) == LevelMap::Item

                        ) &&
                       (*p & LevelMap::BlockWalk) != LevelMap::BlockWalk && (*p & LevelMap::Wall) != LevelMap::Wall && (*p & LevelMap::Object) != LevelMap::Object) {
                SetCollisionData(j, i, 0);
            } else if ((*p & LevelMap::AlternateTile) == LevelMap::AlternateTile && (*p & LevelMap::BlockWalk) != LevelMap::BlockWalk &&
...
int LevelMap::GetMapData(const Point& point, bool abs) const {
    if (abs)
        return GetMapData(AbsToRelative(point), false);
...
bool LevelMap::IsValidPoint(const Point& point, bool abs) const {
    if (abs)
        return IsValidPoint(AbsToRelative(point), false);
    ...
    for (ExitArray::iterator it = begin; it != last; it++) {
        ...
        for (int x = -1; x <= 1; x++) {
            for (int y = -1; y <= 1; y++) {
                ...
                if (IsValidPoint(Point(it->Position.first + x, it->Position.second + y), true) &&
                    SpaceIsWalkable(Point(it->Position.first + x, it->Position.second + y), true)) {
                    blind = false;
                    break;
                }
            }
        ...
        for (int x = -2; x <= 2; x++) {
            for (int y = -2; y <= 2; y++) {
                ...
                if (IsValidPoint(Point(it->Position.first + x, it->Position.second + y), true) &&
                    SpaceHasFlag(LevelMap::ThickenedWall, Point(it->Position.first + x, it->Position.second + y), true)) {
                    SetCollisionData(it->Position.first + x - posX, it->Position.second + y - posY, 0);
                }
            }
        }
...
void LevelMap::FindRoomLinkageExits(ExitArray& exits, RoomList& added) const {
    ...
    do {
        ...
        tmpDistance = GetDistance(it->second.first.first, it->second.first.second, it->second.second.first.first, it->second.second.first.second);
...
bool LevelMap::SpaceHasFlag(int flag, const Point& point, bool abs) const {
    int val = GetMapData(point, abs) | GetMapData(Point(point.first - 1, point.second), abs) | GetMapData(Point(point.first + 1, point.second), abs) |
              GetMapData(Point(point.first, point.second - 1), abs) | GetMapData(Point(point.first, point.second + 1), abs);
...
bool LevelMap::SpaceIsWalkable(const Point& point, bool abs) const {
    ...
    return !(SpaceHasFlag(LevelMap::Avoid, point, abs) || SpaceHasFlag(LevelMap::BlockWalk, point, abs) || SpaceHasFlag(LevelMap::BlockPlayer, point, abs) ||
             SpaceHasFlag(LevelMap::NPCCollision, point, abs) || SpaceHasFlag(LevelMap::Object, point, abs) || SpaceHasFlag(LevelMap::ThickenedWall, point, abs));
}

bool LevelMap::SpaceIsWalkableForExit(const Point& point, bool abs) const {
    // ignore almost everything
    return !(!IsValidPoint(point, abs) || SpaceHasFlag(LevelMap::Avoid, point, abs) || SpaceHasFlag(LevelMap::BlockWalk, point, abs) ||
...
bool LevelMap::ValueIsWalkable(const WORD* value) const {
    return !(ValueHasFlag(LevelMap::BlockWalk, value) || ValueHasFlag(LevelMap::BlockPlayer, value) || ValueHasFlag(LevelMap::Object, value));
}
...
bool LevelMap::SpaceIsInvalid(const Point& point, bool abs) const {
    return SpaceHasFlag(LevelMap::Avoid, point, abs);
}

bool LevelMap::SpaceIsThickenedWall(const Point& point, bool abs) const {
    return SpaceHasFlag(LevelMap::ThickenedWall, point, abs);
}
...
void LevelMap::Dump(const char* file, const PointList& points) const {
    ...
    for (ExitArray::iterator it = exits.begin(); it != exits.end(); it++) {
        ...
        fprintf(f, "Exit %d: at (%d, %d, relative: %d, %d) of type %s linkage to area %s (id: %d, tile id: %d)\n", i++, it->Position.first, it->Position.second,
                rel.first, rel.second, it->Type == Linkage ? "Area" : "Tile", GetLevelIdName(it->Target), it->Target, it->TileId);
    }
